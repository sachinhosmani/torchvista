...
def process_graph(model, inputs, adj_list, module_info, func_info, node_to_module_path, parent_module_to_nodes, parent_module_to_depth, graph_node_name_to_without_suffix, node_to_ancestors, show_non_gradient_nodes, forced_module_tracing_depth):
    ...
    def compress_nested_graph(nested_graph, adj_list, node_to_module_path, graph_node_name_to_without_suffix):
        ...
        def nodes_are_equivalent(node1, node2, adj_list, incoming_edges):
            """
            Checks if two nodes are equivalent (same type and same input/output dims).
            """
            # Get node type/name without suffix
            node1_input_dims = sorted([e['dims'] for e in incoming_edges.get(node1, [])])
            node2_input_dims = sorted([e['dims'] for e in incoming_edges.get(node2, [])])
            
            node1_output_dims = sorted([e['dims'] for e in adj_list[node1]['edges']])
            node2_output_dims = sorted([e['dims'] for e in adj_list[node2]['edges']])
        ...
        def compress_chain(chain, parent_subgraph, adj_list):
            ...
            while i < len(chain):
                ...
                j = i + 1
                while j < len(chain) and nodes_are_equivalent(current_node, chain[j], adj_list, incoming_edges):
                    repeat_count += 1
                    j += 1
                ...
                if repeat_count > 1:
                    ...
                    graph_node_name_to_without_suffix[repeat_name] = f"repeat {repeat_count}x"
                    node_to_module_path[repeat_name] = None
    ...
    compressed_graph = compress_nested_graph(nested_graph, adj_list, node_to_module_path, graph_node_name_to_without_suffix)
